...
export default function SearchResultPage() {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            if (searchData) {
                try {
                    const parsedData = JSON.parse(decodeURIComponent(searchData));
    ...
    return (
        <main className="flex items-center justify-center min-h-screen bg-gray-900 p-4">
            {loading ? (
                ...
                <section className="grid grid-cols-2 grid-rows-4 gap-6 p-10 bg-gray-800 bg-opacity-80 rounded-xl shadow-2xl w-11/12 max-w-6xl">
                    {results.map((result, index) => (
                        <div
                            key={`result-${index}`}
                            className="bg-gray-700 h-24 rounded-2xl border border-gray-600 text-white flex items-center justify-center"
                        >
                            {result}
                        </div>
                    ))}
